...
<template>
  <div class="app-container">
      ...
      <el-button v-if="buttonRole.confJqPermission==1" class="filter-item" style="margin-left: 10px;" @click="openConfJq" type="info" icon="el-icon-setting">{{$t('currency.confJq')}}</el-button>
    ...
    <!-- 配置监区 -->
	<el-dialog :title="$t('currency.confJq')" :visible.sync="dialogJqVisible" width="740px" :modal-append-to-body="false">
  ...
  methods: {
    ...
    setButtonRole() { //设置按钮的权限
    	...
    	if(roles.includes('admin')){
    		...
    		this.buttonRole.confJqPermission= 1
    		...
    		if(holidaySet.length>0){
    			for(let value of holidaySet){
    				if(value=='addPermission'){
    					...
    					this.buttonRole.deletePermission= 1
    				}else if(value=='confJqPermission'){
    					this.buttonRole.confJqPermission= 1
